...
from src.database.routes.orders import (
    get_order_by_id,
)
from src.database.schemas.payments import (
    PaymentCreateSchema,
    RefundRequestSchema,
)
...
async def payments_list(
    ...
    stmt = (
        select(Payment).filter_by(user_id=user_id).order_by(Payment.created_at)
    )
    ...
    if order_db:
        if order_db.user_id == access_token["user_id"]:
            if order_db.status == OrderStatusEnum.PENDING:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Order with id: {order_db.id} paid",
                )
            raise HTTPException(
                status_code=400,
                detail=f"Order with id: {order_db.id} canceled",
            )
        raise HTTPException(
            status_code=403, detail="Current user does not have this order"
        )
    raise HTTPException(
        status_code=404,
        detail=f"Order with id: "
        f"{payment_create_schema.order_id} not in db",
    )
...
async def create_payment_complete(
    ...
    if session["payment_status"] == "paid":
        try:
            order_db = await get_order_by_id(
                id=int(session["metadata"]["order_id"]), db=db
            )
...
async def create_refund_payment(
    ...
    try:
        client.v1.refunds.create(
            {"payment_intent": payment_db.external_payment_id}
        )
