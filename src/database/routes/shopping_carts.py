...
from src.database.schemas.shopping_carts import (
...
carts = APIRouter(prefix="/shopping_carts")


async def get_card_by_user_id(
...
async def add_item_or_create_carts(
    ...
    if not movie_db:
        raise HTTPException(
            status_code=404,
            detail=f"Movie with id: "
            f"{cart_item_schema.movie_id} not in db.",
        )
    if movie_db.is_deleted:
        raise HTTPException(
            status_code=400, detail="You can`t add to cart deleted movie."
        )
    try:
        if not cart_db:
        ...
        paid_movie_stmt = select(Order).filter(
            ...
            Order.order_items.any(
                OrderItem.movie_id == cart_item_schema.movie_id
            ),
        )
        ...
        if paid_movie_db:
            raise HTTPException(
                status_code=400,
                detail=f"Movie with id: "
                f"{cart_item_schema.movie_id} already purchased",
            )
        stmt = select(CartItem).filter_by(
            cart_id=cart_db.id, movie_id=cart_item_schema.movie_id
        )
        result = await db.execute(stmt)
        ...
        if search_card:
            raise HTTPException(
                status_code=400,
                detail=f"Movie with id: "
                f"{cart_item_schema.movie_id} already exist in this cart.",
            )

        await db.refresh(cart_db)
        cart_item_db = CartItem(movie_id=cart_item_schema.movie_id)
...
@carts.delete("/carts/delete_item/{movie_id:int}/", status_code=204)
async def delete_item_from_cart(
    ...
    if not cart_db:
        raise HTTPException(
            status_code=404, detail=f"User with id: {user_id} dont have cart"
        )
