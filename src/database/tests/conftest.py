...
async def create_user(db):
    async def _db_user(
        email: str = "testtest1112111@gmail.com",
        password: str = "Testtest1221!",
    ):
        db_user = User(email=email, group_id=1)
...
async def get_access_token_and_user_id(client, db, create_user):
    async def access_token_obj():
        ...
        response = await client.post(
            ...
            json={"email": test_user.email, "password": "Testtest1221!"},
        )
        return (response.json()["access_token"], test_user.id)
...
async def create_director(db):
    async def director_obj(name: str = "test_director"):
        _db_director = Director(name=name)
        db.add(_db_director)
        await db.commit()
        return _db_director
...
async def create_certification(db):
    async def certification_obj(name: str = "test_certification"):
        _db_certification = Certification(name=name)
...
async def create_rate(db):
    async def rate_obj(movie_id, user_id, rate):
        _db_rate = Rate(movie_id=movie_id, user_id=user_id, rate=rate)
        db.add(_db_rate)
        await db.commit()
        return _db_rate
...
async def create_cart(db):
    async def cart_obj(movie_id, user_id):
        ...
        _db_cart = Cart(user_id=user_id)
        db.add(_db_cart)
        await db.flush()
        _db_cart_item = CartItem(cart_id=_db_cart.id, movie_id=movie_id)
...
async def create_order(db):
    async def order_obj(
        ...
        _db_order = Order(user_id=user_id, status=status, total_amount=price)
        db.add(_db_order)
        await db.flush()
...
async def create_payment(db):
    async def payment_obj(
        ...
        _db_payment_item = PaymentItem(
            price_at_payment=amount, payment_id=12232141, order_item_id=1
        )
