...
async def func_test_with_filtering(client, genre_id, director_id):
    ...
    response = await client.get(f"{movie_prefix}movies/?directors={director_id}")
    decode_response = json.loads(response.content.decode())
    assert response.status_code == 200
    assert len(decode_response) == 1
    assert decode_response[0]["name"] == "Test_movie_name"
...
class TestUnauthorized:
    ...
    @pytest.mark.asyncio
    async def test_movies_list_with_sorting(self, client: AsyncClient, create_movie, create_certification, db):
        certification_db = await create_certification()
    ...
    @pytest.mark.asyncio
    async def test_update_movie_is_staff_required(self, client: AsyncClient, create_movie, create_certification):
        certification_db = await create_certification()
        test_movie = await create_movie(certification_id=certification_db.id)
        response = await client.put(f"{movie_prefix}movies/{test_movie.id}/", json=get_test_movie_data())
    ...
    async def test_create_movie_is_staff_required(self, client: AsyncClient):
        response = await client.post(f"{movie_prefix}movies/", json=get_test_movie_data())
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"

    @pytest.mark.asyncio
    async def test_delete_movie_is_staff_required(self, client: AsyncClient, create_movie, create_certification):
        certification_db = await create_certification()
        test_movie = await create_movie(certification_id=certification_db.id)
        response = await client.delete(f"{movie_prefix}movies/{test_movie.id}/")
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"

    @pytest.mark.asyncio
    async def test_get_favourite_movies_is_authenticated_required(self, client: AsyncClient, create_movie, create_certification):
        certification_db = await create_certification()
        test_movie = await create_movie(certification_id=certification_db.id)
    ...
    @pytest.mark.asyncio
    async def test_add_or_remove_favourite_movies_is_authenticated_required(self, client: AsyncClient, create_movie, create_certification):
        certification_db = await create_certification()
        test_movie = await create_movie(certification_id=certification_db.id)
    ...
    @pytest.mark.asyncio
    async def test_rate_movie_is_authenticated_required(self, client: AsyncClient, create_movie, create_certification):
        certification_db = await create_certification()
        test_movie = await create_movie(certification_id=certification_db.id)
        response = await client.post(
    ...
    @pytest.mark.asyncio
    async def test_like_movie_is_authenticated_required(self, client: AsyncClient, create_movie, create_certification):
        certification_db = await create_certification()
        test_movie = await create_movie(certification_id=certification_db.id)
        response = await client.post(
    ...
    @pytest.mark.asyncio
    async def test_dislike_movie_is_authenticated_required(self, client: AsyncClient, create_movie, create_certification):
        certification_db = await create_certification()
        test_movie = await create_movie(certification_id=certification_db.id)
        response = await client.post(
    ...
    async def test_get_comment_list(
        ...
        result_db = [MovieCommentListResponseSchema.model_validate(comment).model_dump() for comment in comments_db]
    ...
    @pytest.mark.asyncio
    async def test_get_genre_detail(self, client: AsyncClient, create_genre, create_certification, create_movie, db):
    ...
    @pytest.mark.asyncio
    async def test_get_movies_in_genre(self, client: AsyncClient, create_genre, create_certification, create_movie, db):
        test_genre = await create_genre()
        test_certification = await create_certification()
        test_movie = await create_movie(certification_id=test_certification.id)
        test_movie1 = await create_movie(
            name="Test_movie1_name",
            certification_id=test_certification.id,
            genres=[test_genre]
        )
        response = await client.get(f"{movie_prefix}movies_in_genre/{test_genre.id}/")
        decode_response = response.json()
        stmt = select(Movie).filter(Movie.genres.any(Genre.id == test_genre.id))
        result = await db.execute(stmt)
        movies_db = result.unique().scalars().all()
        result_db = [MovieListResponseSchema.model_validate(movie_db).model_dump() for movie_db in movies_db]
        assert response.status_code == 200
        assert result_db == decode_response

    @pytest.mark.asyncio
    async def test_update_genre_is_staff_required(self, client: AsyncClient, create_genre):
        test_genre = await create_genre()
        response = await client.put(f"{movie_prefix}genres/{test_genre.id}/", json={"name": "TestGenre1"})
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"

    @pytest.mark.asyncio
    async def test_create_genre_is_staff_required(self, client: AsyncClient):
        response = await client.post(f"{movie_prefix}genres/", json={"name": "TestGenre1"})
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"

    @pytest.mark.asyncio
    async def test_delete_genre_is_staff_required(self, client: AsyncClient, create_genre):
        test_genre = await create_genre()
        response = await client.delete(f"{movie_prefix}genres/{test_genre.id}/")
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"
    ...
    async def test_get_star_detail(self, client: AsyncClient, create_star, db):
        ...
        result_db = GenreStarDetailResponseSchema.model_validate(star_db).model_dump()
        assert decode_response == result_db

    @pytest.mark.asyncio
    async def test_update_star_is_staff_required(self, client: AsyncClient, create_star):
        test_star = await create_star()
        response = await client.put(f"{movie_prefix}stars/{test_star.id}/", json={"name": "Teststar1"})
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"

    @pytest.mark.asyncio
    async def test_create_star_is_staff_required(self, client: AsyncClient):
        response = await client.post(f"{movie_prefix}stars/", json={"name": "Teststar1"})
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"

    @pytest.mark.asyncio
    async def test_delete_star_is_staff_required(self, client: AsyncClient, create_star):
        test_star = await create_star()
...
class TestAuthorized:
    ...
    async def test_add_remove_and_get_favourite_movies(
        ...
        stmt = select(Movie).filter(Movie.who_add_to_favourite.any(User.id == user_id))
        result = await db.execute(stmt)
        movies_db = result.unique().scalars().all()
        ...
        result_db = [MovieListResponseSchema.model_validate(movie).model_dump() for movie in movies_db]
    ...
    async def test_get_favourite_movies_with_params(
        ...
        stmt = select(Movie).filter(Movie.who_add_to_favourite.any(User.id == user_db.id))
        result = await db.execute(stmt)
        movies_db = result.unique().scalars().all()
        ...
        await func_test_with_filtering(client, genre_db.id, certification_db.id)
        await func_test_with_sorting(client)
    ...
    async def test_rate_movie(
        ...
        test_rate = await create_rate(movie_id=test_movie.id, user_id=user1_db.id, rate=4)
    ...
    async def test_like_dislike_movie(
        ...
        await db.refresh(movie_db)
        assert movie_db.likes_movies == []
        assert movie_db.dislikes_movies[0].movie_id == movie_db.id, "if movie already has dislike, like should delete dislike"
    ...
    async def test_create_movie_comment(
        ...
        assert response.json() == CommentResponseSchema.model_validate(comment_db).model_dump()
    ...
    async def test_reply_movie_comment(
        ...
        assert response.json() == ReplyCommentResponseSchema.model_validate(reply_comment_db).model_dump()
    ...
    async def test_like_dislike_comment(
        ...
        await db.refresh(comment_db)
        assert comment_db.likes_comments == []
        assert comment_db.dislikes_comments[0].comment_id == comment_db.id, "if comment already has dislike, like should delete dislike"
...
class TestModerator:
    ...
    async def test_create_movie(
        ...
        result = await db.execute(stmt)
        movie_db = result.unique().scalar_one_or_none()
        assert MovieDetailResponseSchema.model_validate(movie_db).model_dump() == response.json()
        ...
        assert duplicate_response.json()["detail"] == f"Movie with name: {get_test_movie_data()["name"]}, year: {get_test_movie_data()["year"]}, time: {get_test_movie_data()["time"]} already exist in db."
    ...
    async def test_update_movie(
        ...
        update_response = await client.put(
            ...
            headers={"Authorization": f"Bearer {access_token}"},
            json=get_test_movie_data(name="Test_movie2", certification_id=test_certification.id)
        )
        await db.refresh(test_movie)
        stmt = select(Movie)
        result = await db.execute(stmt)
    ...
    async def test_create_star(
        ...
        assert response.status_code == 200
        assert response.json() == GenreStarDetailResponseSchema.model_validate(stars_db).model_dump()
    ...
    async def test_update_star(
        ...
        assert response.json() == GenreStarDetailResponseSchema.model_validate(test_star).model_dump()
    ...
    async def test_create_genre(
        ...
        assert response.status_code == 200
        assert response.json() == GenreStarDetailResponseSchema.model_validate(genres_db).model_dump()
    ...
    async def test_update_genre(
        ...
        assert response.json() == GenreStarDetailResponseSchema.model_validate(test_genre).model_dump()
