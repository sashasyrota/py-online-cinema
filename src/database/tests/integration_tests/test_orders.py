...
class TestUnauthorized:
    @pytest.mark.asyncio
    async def test_create_order_is_auth_required(
            self,
            client
    ):
...
class TestAuthorized:
    ...
    async def test_create_order(
        ...
        assert response.content.decode() == OrderResponseDetailSchema.model_validate(order_db).model_dump_json()
    ...
    async def test_cancel_order(
        ...
        access_token, user_id = await get_access_token_and_user_id()
        cart_db = await create_cart(movie_id=movie_db.id, user_id=user_id)
        order_create_response = await client.post(
            f"{order_prefix}create_order/",
            headers={"Authorization": f"Bearer {access_token}"}
        )
        stmt = select(Order).filter_by(user_id=user_id)
        ...
        assert cancel_pending_order_response.content.decode() == OrderResponseDetailSchema.model_validate(order_db).model_dump_json()
        ...
        assert cancel_paid_order_denied_response.json()["detail"] == 'You can cancel only pending order.'