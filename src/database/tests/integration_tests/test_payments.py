...
class TestAuthorized:
    ...
    async def test_create_payment(
        ...
        with patch("src.database.routes.payments.client.v1.checkout.sessions.retrieve") as mocked_stripe:
            ...
            assert complete_response.json() == {"Payment complete successful": True}
    ...
    async def test_create_payment_with_purchased_and_canceled_order_denied(
        ...
        assert request_response.json()["detail"] == f"Order with id: {order_db.id} paid"
        ...
        assert request_response.status_code == 400
        assert request_response.json()["detail"] == f"Order with id: {order_db.id} canceled"
    ...
    async def test_create_payment_with_incorrect_order_id_denied(
        ...
        assert request_response.json()["detail"] == "Order with id: 1 not exist in db"
    ...
    async def test_create_payment_with_order_another_user_denied(
        ...
        cart_db = await create_cart(movie_id=movie_db.id, user_id=user1.id)
        order_db = await create_order(
            user_id=user1.id,
            movie_id=movie_db.id,
            price=movie_db.price,
        ...
        assert request_response.json()["detail"] == "Current user does not have this order"
    ...
    async def test_refund_payment(
        ...
        assert payment_not_found_response.json()["detail"] == "Payment not found"
        assert payment_not_found_response.status_code == 404
        payment_db = await create_payment(
            user_id=user_id,
            order_id=order_db.id,
            amount=movie_db.price
        )

        with patch("src.database.routes.payments.client.v1.refunds.create") as mocked_stripe:
            successful_refund_response = await client.post(
                f"{payment_prefix}refund_payment/",
                json={"order_id": order_db.id},
                headers={"Authorization": f"Bearer {access_token}"}
            )
            assert successful_refund_response.json() == {'Payment was returned': True}
            assert mocked_stripe.mock_calls[0].args[0] == {'payment_intent': str(payment_db.external_payment_id)}
            ...
            assert duplicate_refund_response.json()["detail"] == "Payment not found"
