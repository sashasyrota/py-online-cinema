...
async def get_account_response(
    ...
    response = await client.post(f"{account_prefix}{route_url}", json=body_params, headers=headers, data=data, files=files)
    return response
...
class TestUnauthorized:
    ...
    async def test_account_register_fields(self, client: AsyncClient, db):
        ...
        assert decode_response["detail"] == "Password should consist one of these symbols {'/.!&?,'}"
        ...
        decode_response = json.loads(response.content.decode())
        assert response.status_code == 400
        assert decode_response["detail"] == "Password should consist at least one digit"
        ...
        decode_response = json.loads(response.content.decode())
        assert response.status_code == 400
        assert decode_response["detail"] == "Password should be not shorter than 8 characters"
        ...
        decode_response = json.loads(response.content.decode())
        assert response.status_code == 400
        assert decode_response["detail"] == "Password should consist at least one upper-case letter"
        ...
        decode_response = json.loads(response.content.decode())
        assert response.status_code == 400
        assert decode_response["detail"] == "Password should consist at least one lower-case letter"
    ...
    @pytest.mark.asyncio
    async def test_account_reset_request_and_complete(self, client, db, create_user):
        db_user = await create_user()
        response = await client.post("api/v1/accounts/reset-request/", json={"email": db_user.email})
        decode_response = json.loads(response.content.decode())
        assert decode_response["detail"] == "User account is not active"
        assert response.status_code == 403
        ...
        response_valid_token = await get_account_response(
            ...
            body_params={"reset_token": decode_response["reset_token"], "password": "NewPassword1221!"}
        )
        ...
        response_invalid_token = await get_account_response(
            client=client,
            route_url="reset-complete/",
            body_params={"reset_token": "invalid_token", "password": "NewPassword1221!"}
        )
        ...
        response_invalid_token = await get_account_response(
            ...
            route_url="reset-complete/",
            body_params={
                "reset_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoxfQ.SHRALNlfZeIZv30Zxkhmf74jOwgo8XAkD5MPky6eT0Y",
    ...
    async def test_refresh_token(self, client, db, create_user):
        ...
        response_refresh_token = await get_account_response(
            client=client,
            route_url="refresh/",
            body_params={"refresh_token": decode_response_login["refresh_token"]}
        )
        decode_response_refresh_token = json.loads(response_refresh_token.content.decode())
...
class TestAuthorized:
    @pytest.mark.asyncio
    async def test_create_user_profile(self, client, create_user, db, image_in_memory):
        db_user = await create_user()
        db_user.is_active = True
        await db.commit()
        response_login = await get_account_response(
        ...
        decode_response_create_profile_without_avatar = json.loads(response_create_profile_without_avatar.content.decode())
        assert decode_response_create_profile_without_avatar["detail"] == [{"type":"missing","loc":["body","avatar"],"msg":"Field required","input": None}]
        ...
        decode_response_create_profile_without_info = json.loads(response_create_profile_without_info.content.decode())
        assert decode_response_create_profile_without_info["detail"] == [{"type":"missing","loc":["body","info"],"msg":"Field required","input":None}]
        ...
        stmt_user_profile = select(UserProfile).filter(UserProfile.user_id == db_user.id)
        ...
        decode_response_successful_create_profile = json.loads(response_successful_create_profile.content.decode())
    ...
    @pytest.mark.asyncio
    async def test_change_account_state_is_admin_required(self, client, create_user, db):
        db_user = await create_user()
        db_user.is_active = True
        await db.commit()
        ...
        decode_user_change_state_response = json.loads(user_change_state_response.content.decode())

        assert user_change_state_response.status_code == 403
        assert decode_user_change_state_response["detail"] == "Not enough permissions for this operation"
...
class TestIsAdmin:
    ...
    async def test_change_account_state_is_admin_required(self, client, create_user, db):
        ...
        assert db_user1.is_active == True
