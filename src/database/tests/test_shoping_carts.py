...
class TestUnauthorized:
    ...
    async def test_add_item_is_auth_required(
        ...
        response = await client.post(
            f"{cart_prefix}carts/add_item/", json={"movie_id": 1}
        )
        assert response.status_code == 401
        assert response.json()["detail"] == "Not authorized"
...
class TestAuthorized:
    ...
    async def test_get_cart_detail(
        ...
        assert (
            ...
            == CartDetailResponseSchema.model_validate(cart_db)
            .model_dump_json()
        )
    ...
    async def test_add_item(
        ...
        movie_db1 = await create_movie(
            certification_id=certification_db.id, name="TESTMOVIE1"
        )
        ...
        assert (
            CartListResponseSchema.model_validate(cart_db).model_dump_json()
            == no_cart_response.content.decode()
        )
        ...
        await db.refresh(cart_db)
        assert (
            add_item_to_existing_cart_response.content.decode()
            == CartListResponseSchema.model_validate(cart_db).model_dump_json()
        )
    ...
    async def test_add_already_bought_movie_denied(
        ...
        assert (
            add_bought_item_to_cart_response.json()["detail"]
            == f"Movie with id: {movie_db.id} already purchased"
        )
...
class TestModerator:
    ...
    @pytest.mark.asyncio
    async def test_sent_message_func(self):
        content = "TestContent"
    ...
    async def test_notify_users_in_background(
        ...
        with mock.patch(
            "src.database.routes.movies.BackgroundTasks.add_task"
        ) as mocked_obj:
